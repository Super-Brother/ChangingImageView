package com.wenchao.changingimageview;
 ...
 * @author zhang
 */
public class ChangingImageView extends AppCompatImageView {

    private static final String TAG = ChangingImageView.class.getSimpleName();
    /**
     * 未指定缩放比例
     */
    private static final float UNSPECIFIED_SCALE_FACTOR = -1f;
    /**
     * 回弹动画事件
     */
    private int mRevertDuration = 300;
    /**
     * 最大缩放比例
     */
     ...
     * 此最小缩放比例优先级高于下面两个
     */
    private float mMinScaleFactor = UNSPECIFIED_SCALE_FACTOR;
    /**
     * 图片最初的最小缩放比例
     */
    private float mVerticalMinScaleFactor = 1.0f;
    /**
     * 图片旋转90（或-90）度后的的最小缩放比例
     */
    private float mHorizontalMinScaleFactor = 1.0f;
    /**
     * 默认适应控件大小后的初始化缩放比例
     */
     ...
     */
    protected RectF mImageRect = new RectF();
    /**
     * 是否开启缩放回弹
     */
    private boolean mOpenScaleRevert = false;
    /**
     * 是否开启旋转回弹
     */
    private boolean mOpenRotateRevert = false;
    /**
     * 是否开启平移回弹
     */
    private boolean mOpenTranslateRevert = false;
    /**
     * 是否开启动画
     */
    private boolean mOpenAnimator = false;
    /**
     * 回弹动画
     */
    private MatrixRevertAnimator mRevertAnimator = new MatrixRevertAnimator();
    /**
     * 当前各触点的中点
     */
    private PointF mCurrentMidPoint = new PointF();

    public ChangingImageView(Context context) {
        this(context, null);
    }

    public ChangingImageView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public ChangingImageView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void getAttrs(AttributeSet attrs) {
        ...
        TypedArray typedArray = getContext().obtainStyledAttributes(attrs, R.styleable.ChangingImageView);
        mMaxScaleFactor = typedArray.getFloat(
                R.styleable.ChangingImageView_max_scale, mMaxScaleFactor);
        mMinScaleFactor = typedArray.getFloat(
                R.styleable.ChangingImageView_min_scale, UNSPECIFIED_SCALE_FACTOR);
        mRevertDuration = typedArray.getInteger(
                R.styleable.ChangingImageView_revert_duration, mRevertDuration);
        mOpenScaleRevert = typedArray.getBoolean(
                R.styleable.ChangingImageView_open_scale_revert, false);
        mOpenRotateRevert = typedArray.getBoolean(
                R.styleable.ChangingImageView_open_rotate_revert, false);
        mOpenTranslateRevert = typedArray.getBoolean(
                R.styleable.ChangingImageView_open_translate_revert, false);
        mOpenAnimator = typedArray.getBoolean(
                R.styleable.ChangingImageView_open_animator, true);
        mScaleBy = typedArray.getInt(
                R.styleable.ChangingImageView_scale_center, SCALE_BY_IMAGE_CENTER);
        typedArray.recycle();
    }
    ...
    private void initImagePositionAndSize() {
        ...
        //计算缩放比例，使图片适应控件大小
        mHorizontalMinScaleFactor = Math.min(getWidth() / mImageRect.width(), getHeight() / mImageRect.height());
        mVerticalMinScaleFactor = Math.min(getHeight() / mImageRect.width(), getWidth() / mImageRect.height());
     ...
     * 图片是否正在变化
     */
    protected boolean isTransforming = false;
        ...
        switch (event.getActionMasked()) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                if (mOpenRotateRevert || mOpenScaleRevert || mOpenTranslateRevert) {
                    ...
                    mMatrix.getValues(mToMatrixValue);
                    if (mOpenAnimator) {
                        // 启动回弹动画
    ...
    private void checkScale() {
        ...
        // 获取图片当前是水平还是垂直
        int imgOrientation = getImageOrientation();
        // 超过设置的上限或下限则回弹到设置的限制值
        // 除以当前图片缩放比例mScaleFactor，postScale()方法执行后的图片的缩放比例即为被除数大小
        if (imgOrientation == HORIZONTAL && mScaleFactor < mHorizontalMinScaleFactor) {
            scaleFactor = mHorizontalMinScaleFactor / mScaleFactor;
        } else if (imgOrientation == VERTICAL && mScaleFactor < mVerticalMinScaleFactor) {
    ...
    private void scale(MotionEvent event) {
        ...
        mScaleFactor *= scaleFactor;
        mMatrix.postScale(scaleFactor, scaleFactor, scaleCenter.x, scaleCenter.y);
        mLastPoint2.set(mCurrentPoint1);
        ...
        public void onAnimationUpdate(ValueAnimator valueAnimator) {
            if (mFromMatrixValue != null && mToMatrixValue != null && mInterpolateMatrixValue != null) {
                ...
                for (int i = 0; i < 9; i++) {
                    ...
                    mInterpolateMatrixValue[i] = mFromMatrixValue[i] + (mToMatrixValue[i] - mFromMatrixValue[i]) * animatedValue;
                }
